...
from .controllers import orders, recipes, sandwiches, resources, order_details
...
@app.get("/sandwiches/{sandwich_name}", response_model=schemas.Sandwich, tags=["Sandwiches"])
def read_one_sandwich(sandwich_name: str, db: Session = Depends(get_db)):
...
def delete_one_sandwich(sandwich_name: str, db: Session = Depends(get_db)):
    ...
    if sandwich is None:
        raise HTTPException(status_code=404, detail="Sandwich not found")
    return sandwiches.delete(db=db, sandwich_name=sandwich_name)
...
def delete_one_resource(item: str, db: Session = Depends(get_db)):
    ...
    if resource is None:
        raise HTTPException(status_code=404, detail="Resource not found")
    return resources.delete(db=db, item=item)
...
def delete_one_order_detail(order_id: int, db: Session = Depends(get_db)):
    order_detail = order_details.read_one(db, order_id=order_id)
    if order_detail is None:
        raise HTTPException(status_code=404, detail="Order Details not found")
